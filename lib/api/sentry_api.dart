...
class SentryApi {
  ...
  Future<List<Organization>> organizations() async {
    ...
    final response = await client.get(
        Uri.https(baseUrlName, '$baseUrlPath/organizations/', queryParameters),
        headers: _defaultHeader());
    return _parseResponseList(
        response, (jsonMap) => Organization.fromJson(jsonMap));
  }

  Future<Organization> organization(String organizationSlug) async {
    final response = await client.get(
        Uri.https(baseUrlName, '$baseUrlPath/organizations/$organizationSlug/'),
        headers: _defaultHeader());
  ...
  Future<List<Project>> projects(String? slug) async {
    final response = await client.get(
        Uri.https(baseUrlName, '$baseUrlPath/organizations/$slug/projects/'),
        headers: _defaultHeader());
  ...
  Future<Project> project(String organizationSlug, String projectSlug) async {
    final response = await client.get(
        Uri.https(baseUrlName,
            '$baseUrlPath/projects/$organizationSlug/$projectSlug/'),
        headers: _defaultHeader());
  ...
  Future<Set<String>> projectIdsWithSessions(String organizationSlug) async {
    ...
    final request = Uri.https(baseUrlName,
            '$baseUrlPath/organizations/$organizationSlug/sessions/')
        .resolveUri(Uri(queryParameters: queryParameters));

    final response = await client.get(request, headers: _defaultHeader());

    final sessions =
        _parseResponse(response, (jsonMap) => Sessions.fromJson(jsonMap!));
  ...
  Future<Project> bookmarkProject(
      String organizationSlug, String projectSlug, bool bookmark) async {
  ...
  Future<List<Release>> releases(
      ...
      int health = 1,
      int flatten = 0,
      String summaryStatsPeriod = '24h'}) async {
    ...
    final response = await client.get(
        Uri.https(
            baseUrlName,
            '$baseUrlPath/organizations/$organizationSlug/releases/',
            queryParameters),
        headers: _defaultHeader());
  ...
  Future<Release> release(
    ...
    final response = await client.get(
        Uri.https(
            baseUrlName,
            '$baseUrlPath/organizations/$organizationSlug/releases/$releaseId/',
            queryParameters),
        headers: _defaultHeader());
    return _parseResponse(response, (jsonMap) => Release.fromJson(jsonMap));
  }

  Future<List<Group>> issues(
      {required String? organizationSlug, required String? projectSlug}) async {
    final queryParameters = {'statsPeriod': '24h'};
    final response = await client.get(
        Uri.https(
            baseUrlName,
            '$baseUrlPath/projects/$organizationSlug/$projectSlug/issues/',
            queryParameters),
        headers: _defaultHeader());
  ...
  Future<double?> apdex(
    ...
    final response = await client.get(
        Uri.https(
            baseUrlName,
            '$baseUrlPath/organizations/$organizationSlug/eventsv2/',
            queryParameters),
        headers: _defaultHeader());
  ...
  Future<User> authenticatedUser() async {
    ...
    return _parseResponse(response,
        (jsonMap) => User.fromJson(jsonMap!['user'] as Map<String, dynamic>));
  }
